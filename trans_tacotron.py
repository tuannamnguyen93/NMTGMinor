...
def main():
    ...
    src_lang = int(checkpoint['dicts']['langs'][opt.src_lang])
    model = build_model(model_opt, checkpoint['dicts'])[0]
    ...
    output_ark = opt.output + "/" + opt.output_name +".ark"
    output_scp = opt.output + "/" + opt.output_name + ".scp"
    ...
    if opt.encoder_type == "audio":
        ...
        while True:
            ...
            print(src_lang)
            src_lang = torch.tensor(src_lang).long()
            outputs = model.inference(tensor.unsqueeze(0).half().cuda(), src_lang.unsqueeze(0).cuda())

            mel = outputs[0].squeeze(0).transpose(0, 1).float().cpu().detach().numpy()
            print(mel.shape)
            dic = {seg_name: mel}
            append = False if i == 1 else True

            write_ark(output_ark, dic, output_scp, append=append)
            # print(outputs[0].shape)
            # audio = waveglow.infer(outputs[0][:,:,:800].float().cpu())
            # print(audio.shape)
            #
            # audio_numpy = audio[0].data.cpu().numpy()
            # rate = 22050
            # write("audio_chinese_to_indian"+seg_name+".wav", rate, audio_numpy)
            if i > 10:
                break
